...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ videoId: string }> }) {
    ...
    try {
        const { status, encodeProgress, lengthSec, missing } = await getBunnyVideoStatus(videoId)
        return NextResponse.json({ status, encodeProgress, lengthSec, missing: Boolean(missing) })
    } catch (error) {
